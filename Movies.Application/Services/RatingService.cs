...
public class RatingService : IRatingService
{
    ...
    {
        return await _ratingRepository.DeleteAsync(movieId, userId, token);
    }

    public async Task<IEnumerable<MovieRating>> GetRatingsAsync(Guid userId, CancellationToken token = default)
    {
        return  await _ratingRepository.GetRatingsAsync(userId, token);
    }
}